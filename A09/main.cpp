...
/*
 * #include "models.ccp"
 * ASSIGNMENT MY CODE, PROF VERTEX DEFINITION MOVED IN models.h
 */
namespace std {
...
private:
	...
	VertexDescriptor 	geometryVertices = VertexDescriptor(true, false, false, false, false);
	...
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<Assignment04*>
...
//			VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | // disabled in the tutorial
	...
	void createPhongDescriptorSetLayout() {
		...
		std::array<VkDescriptorSetLayoutBinding, 2> bindings =
							{uboLayoutBinding, globalUboLayoutBinding};
	...
	void createTextDescriptorSetLayout() {
 	...
 	void createPhongPipeline() {
 		createPipeline("PhongVert.spv", "PhongFrag.spv",
 					    ...
 					    VK_CULL_MODE_NONE, false, geometryVertices);
 		createPipeline("PhongVert.spv", "PhongFrag.spv",
...
//throw std::runtime_error("Now We Stop Here!");			
	...
	void createModels() {
		createModelMesh(Scene[0].MD, geometryVertices);
		createVertexBuffer(Scene[0].MD);
		createIndexBuffer(Scene[0].MD);
	}
...
//std::cout << k << " " << j << " " << i << " " << vb << " " << ib << " " << c << "\n";
	...
	void createModelMesh(ModelData& MD, VertexDescriptor &VD) {
		MD.vertDesc = &VD;
		
		makeModels();
		
		MD.vertices.resize(M1_vertices.size() + M2_vertices.size() + M3_vertices.size() + M4_vertices.size());
		MD.indices.resize(M1_indices.size() + M2_indices.size() + M3_indices.size() + M4_indices.size());
		
		int stV = 0, stI = 0;
		for(int i = 0; i < M1_vertices.size(); i++) {
			MD.vertices[stV+i] = M1_vertices[i];
		}
		for(int i = 0; i < M1_indices.size(); i++) {
			MD.indices[stI+i] = M1_indices[i] + stV/3;
		}
		Mstart[0] = stI;
		Mlen[0] = M1_indices.size();
		stV += M1_vertices.size();		
		stI += M1_indices.size();
		for(int i = 0; i < M2_vertices.size(); i++) {
			MD.vertices[stV+i] = M2_vertices[i];
		}
		for(int i = 0; i < M2_indices.size(); i++) {
			MD.indices[stI+i] = M2_indices[i] + stV/3;
		}
		Mstart[1] = stI;
		Mlen[1] = M2_indices.size();
		stV += M2_vertices.size();		
		stI += M2_indices.size();
		for(int i = 0; i < M3_vertices.size(); i++) {
			MD.vertices[stV+i] = M3_vertices[i];
		}
		for(int i = 0; i < M3_indices.size(); i++) {
			MD.indices[stI+i] = M3_indices[i] + stV/3;
		}
		Mstart[2] = stI;
		Mlen[2] = M3_indices.size();
		stV += M3_vertices.size();		
		stI += M3_indices.size();
		for(int i = 0; i < M4_vertices.size(); i++) {
			MD.vertices[stV+i] = M4_vertices[i];
		}
		for(int i = 0; i < M4_indices.size(); i++) {
			MD.indices[stI+i] = M4_indices[i] + stV/3;
		}
		Mstart[3] = stI;
		Mlen[3] = M4_indices.size();
		stV += M4_vertices.size();		
		stI += M4_indices.size();

		std::cout << "Total components: " << MD.vertices.size()
				  << ", Total indices: " << MD.indices.size() << "\n";
		std::cout << "Total vertices: " << MD.vertices.size() / 3
				  << ", Total triangles: " << MD.indices.size() / 3 << "\n";
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			}
			{
				...
				vkCmdDrawIndexed(commandBuffers[i],
							static_cast<uint32_t>(Mlen[curText]), 1, static_cast<uint32_t>(Mstart[curText]), 0, 0);
			}
