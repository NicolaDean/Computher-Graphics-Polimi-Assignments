...
std::vector<SingleText> SceneText = {
	{1, {"PO1 - Central, FovY = 90o", "", "", ""}, 0, 0},
	{1, {"PO2 - Central, FovY = 120o", "", "", ""}, 0, 0},
	{1, {"PO3 - Central, FovY = 30o", "", "", ""}, 0, 0},
	{3, {"PO4 - Only the left side, FovY = 90o",
		 "Here r=0, and l, t, and b should be",
		 "computed to match aspect ratio and FovY", ""}, 0, 0}};
...
private:
	...
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<Assignment05*>
...
//		Prj[1][1] *= -1;
		...
		switch(curText) {
		  case 0:
			Prj = PO1(swapChainExtent.width / (float) swapChainExtent.height);
			if(curText != prevCt) {
				std::cout << "Using PO1(" << (swapChainExtent.width / (float) swapChainExtent.height) <<")\n";
				prevCt = curText;
			}
			break;
		  case 1:
			Prj = PO2(swapChainExtent.width / (float) swapChainExtent.height);
			if(curText != prevCt) {
				std::cout << "Using PO2(...)\n";
				prevCt = curText;
			}
			break;
		  case 2:
			Prj = PO3(swapChainExtent.width / (float) swapChainExtent.height);
			if(curText != prevCt) {
				std::cout << "Using PO3(...)\n";
				prevCt = curText;
			}
			break;
		  case 3:
			Prj = PO4(swapChainExtent.width / (float) swapChainExtent.height);
			if(curText != prevCt) {
				std::cout << "Using PO4(...)\n";
				prevCt = curText;
			}
