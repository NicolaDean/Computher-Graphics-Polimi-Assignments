...
std::vector<SingleText> SceneText = {
	{4, {"MT1 - Rotate 45 degrees around an arbitrary axis",
	     "passing through (1,0,-1). The x-axis can be aligned",
	     "to the arbitrary axis after a rotation of 30 deg.",
	     "around the z-axis, and then -60 deg. around the y-axis"}, 0, 0},
	{2, {"MT2 - Half the size of an object, using", "as fixed point (5,0,-2).", "", ""}, 0, 0},
	{3, {"MT3 - Mirror the starship along a plane passing",
		 "through (1,1,1), and obtained rotating 15 degree","around the x axis the xz plane", ""}, 0, 0},
	{4, {"MT4 - Apply the inverse of the following sequence of",
	     "transforms: Translation of (0, 0, 5) then rotation", "of 30 degree around the Y axis,",
	     "and finally a uniform scaling of a factor of 3."}, 0, 0}};

std::vector<glm::mat4> VisMT;
...
//										glm::vec3(0.0f, 1.0f, 0.0f));
				ubo.mMat = VisMT[curText];			
			}
			ubo.mMat = glm::scale(glm::mat4(1), glm::vec3(SceneToLoad[j].scale)) * ubo.mMat;
